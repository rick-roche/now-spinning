...
async function loadCollectionSnapshot(params: {
  ...
  const { storage, sessionId, username, authHeader } = params;
  const snapshotCacheKey = `discogs:collection:index:${CACHE_VERSION}:${sessionId}`;
  const cachedSnapshot = storage.getCache<DiscogsCollectionSnapshot>(snapshotCacheKey);
  ...
  const buildPromise: Promise<SnapshotLoadResult> = (async () => {
    ...
    storage.setCache(snapshotCacheKey, snapshot, COLLECTION_SNAPSHOT_TTL_SECONDS);
...
router.get("/collection", async (c: HonoContext) => {
  ...
  let identity = storage.getCache<DiscogsIdentityResponse>(identityCacheKey);
  ...
  if (!identity?.username) {
    ...
    storage.setCache(identityCacheKey, identity, CACHE_TTL_SECONDS);
  }
  ...
  if (useDiscogsDirectPath) {
    ...
    const cached = storage.getCache<DiscogsCollectionResponse>(cacheKey);
    if (cached) {
  ...
  const cachedSearch = storage.getCache<DiscogsCollectionResponse>(searchCacheKey);
  if (cachedSearch) {
  ...
  storage.setCache(searchCacheKey, response, CACHE_TTL_SECONDS);
